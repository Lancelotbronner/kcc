...
memmap *overlay_ = NULL;     /* overlay segment             */
...
void allocLocal(symbol *sym) {
  ...
  if ((SPEC_SCLS(sym->etype) == S_DATA) ||
    ...
    SPEC_OCLS(sym->etype) = (options.noOverlay ? data : overlay_);
    allocIntoSeg(sym);
    return;
  }
...
void overlay2data() {
  symbol *sym;
	...
	for (sym = setFirstItem(overlay_->syms); sym;
       sym = setNextItem(overlay_->syms)) {

    SPEC_OCLS(sym->etype) = data;
    allocIntoSeg(sym);
  }

  setToNull((void *)&overlay_->syms);
}
...
#define BREAKLINE                                                              \
  ...
  tempOverlaySyms = overlay_->syms;

  /* search the set of overlay sets for local variables/parameters */
  for (ovrset = setFirstItem(ovrSetSets); ovrset;
       ovrset = setNextItem(ovrSetSets)) {
    overlay_->syms = ovrset;
    cnt += printAllocInfoSeg(overlay_, func, oBuf);
  }
  overlay_->syms = tempOverlaySyms;
