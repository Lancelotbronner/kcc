...
bool scan_varlen(int c) {
	...
	// the character isn't part of any recognised token, emit an error
	char source[2] = { Character, 0 };
	diagnostic_code("LEX", 1);
	diagnostic_level(DIAGNOSTIC_ERROR);
	diagnostic_location(source, (uint32_t)Line, (uint32_t)Column);
	diagnostic_snippet(0, 1, "unrecognized character");
	diagnostic_emit();
...
int next() {
	if (PutbackIndex) {
		Character = Putback[--PutbackIndex];
		return Character;
	}
